...
pub fn default_path(id: Uuid) -> PathBuf {
    PathBuf::from(format!("microvisor-{id}.yaml"))
}

pub fn render(id: Uuid) -> String {
    format!(
        r#"# Replace the name and absolute paths before installing this profile.
schema_version: 1
id: {id}
...
pub fn write_new(path: &Path, id: Uuid) -> Result<()> {
    if path.extension().and_then(|extension| extension.to_str()) != Some("yaml") {
        bail!("Generated profile file names must end with '.yaml'");
    }
    ...
    let mut file = OpenOptions::new()
        ...
        .open(path)
        .with_context(|| format!("Could not create profile template {}", path.display()))?;
    let result = file
    ...
    if let Err(error) = result {
        ...
        return Err(error)
            .with_context(|| format!("Could not write profile template {}", path.display()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn renders_a_parseable_profile_with_the_requested_uuid() {
        ...
        let text = render(id);
        let profile = config::parse_profile(&text).unwrap();
    ...
    fn requires_the_loader_visible_yaml_extension() {
        let root = TempDir::new().unwrap();
        assert!(write_new(&root.path().join("profile.yml"), Uuid::new_v4()).is_err());
    }
}
